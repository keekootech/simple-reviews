...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  console.log(`Found ${dueRequests.length} review email(s) due.`);
...
for (const req of dueRequests) {
   ...
   const logoHtml = settings?.logoUrl
        ...
        <div style="max-width: 560px; margin: 0 auto; background: #ffffff; border-radius: 12px; overflow: hidden; border: 1px solid #ececec;">
          ...
          <div style="padding: 32px;">
            ...
            <div style="text-align: center; margin-bottom: 8px;">
              <a href="${reviewUrl}" style="display: inline-block; padding: 14px 36px; background: ${btnColor}; color: #ffffff; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 15px;">
                Leave a Review
              </a>
            </div>

            <div style="text-align: center; margin-top: 8px;">
              <span style="font-size: 22px; letter-spacing: 4px; color: #ddd;">☆☆☆☆☆</span>
            </div>
          </div>

          <div style="padding: 20px 32px; background: #fafafa; border-top: 1px solid #f0f0f0; text-align: center;">
            <p style="font-size: 12px; color: #999; margin: 0;">
              Order #${req.orderNumber} · You're receiving this because you recently made a purchase.
            </p>
          </div>
      ...
      console.log(`Sent review email for order ${req.orderNumber}`);
    } catch (error) {
      console.error(`Failed to send for order ${req.orderNumber}:`, error);
    }
  }

  return new Response(JSON.stringify({ processed: dueRequests.length }), {
